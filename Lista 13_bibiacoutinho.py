...
def x_antes(words):
  ...
  for i in words:
    if i.startswith('x'):
      ...
      listaresto.append(i)
  listax = sorted(listax)
  listaresto = sorted(listaresto)
  return listax + listaresto

def last(a): #esta def serve para a letra C
  return a[-1] ##função definida para encontrar último termo
...
def sort_last(tuples):
  return sorted(tuples,key=last) ##ordena o 'tuples' de acordo com 
                                ##a ordem crescente dos últimos elementos
