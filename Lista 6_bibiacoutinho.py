...
def apaga(s, n):
  ...
  for a in s:
    if a != s[n]:
      palavra = palavra + a ##vai adicionando na string todas as letras que são diferentes da letra posição: s(n)
  return palavra
...
def troca(s):
  if len(s) <= 1:
    ...
    return s[-1] + s[1:-1] + s[0] ##ao invés de trocar a primeira pela última e vice-versa, eu "ditei" pro computador o que ele deveria escrever
